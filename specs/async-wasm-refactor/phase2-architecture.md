...
```
  ...
  Shape: `join!(process_service_messages(), async { let r = select!(wait_to_finish, evaluate); finalize_primary_progress(); send(JobFinishing); r })`, then `finish_run_with_result`. `tokio::select!`/`futures::join!` are executor-agnostic (no reactor needed) — they work on wasm. `finish_run_with_result` currently takes `Result<Result<(),Error>, JoinError>` (assets.rs:1328); it is refactored to take `Result<(), Error>` for the psm outcome, with the `Queued` caller mapping `JoinError` → `Error` at the call site (small seam, Phase 4).
- **`MetadataSaver`** (assets.rs:145): `save_immediately` matches on the asset's `EvalMode`: `Queued` → current debounced spawn path; `Inline` → direct `store.set_metadata(...).await` (no spawn, no `sleep`, **no `std::time::Instant`** — removing the wasm panic hazard from the inline path).
