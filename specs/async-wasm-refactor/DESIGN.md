...
- **M-D/M-E/M-F REMAINING (next):** M-D parametric tests + test-support `ImmediateEnvironment` (proves ImmediateAssetManager evaluates correctly at runtime on native); M-E `trunk build` + Playwright e2e (runtime acceptance in Chromium); M-F docs/ISSUES.
