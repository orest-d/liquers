...
- **M-C/M-D/M-E/M-F REMAINING (next):** macro (registration.rs:1118 `BoxFuture`); liquers-lib `DefaultEnvironment` cfg-select ImmediateAssetManager + `init_with_envref` split + A2 `Instant::now()` audit + dual cfg_attr on impls; liquers-py/axum conformance; parametric tests + test-support `ImmediateEnvironment`; Playwright e2e; docs. Checkpoint M-C: native workspace + `cargo check --target wasm32 -p liquers-lib`.
