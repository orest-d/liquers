...
1. **Scope:** ~~Tier 1 only or Tier 1 + Tier 2?~~ **RESOLVED (user, 2026-07-22): Tier 1 first, then decide.** Tier 2 (conditional-`Send`) is deferred and re-evaluated after Tier 1 lands and the browser example runs. *Superseding note:* Tier 1 was initially envisioned as a cfg-gated `rt` spawn/timer shim; the subsequently-chosen b1 strategy (`ImmediateAssetManager`, see Strategy Evaluation) achieves Tier 1 **without any shim** — the inline path contains no spawns and no timers at all. All `Send` bounds are still kept; downstream ripple is limited to the `Environment::get_asset_manager` generalization.
