---
id: ASYNC-WASM-REFACTOR
kind: design
title: Async evaluation on wasm
status: complete
...
- **M-B DONE (green, committed) ✅** — checkpoint reached: `cargo test -p liquers-core` = **324+ tests pass, 0 failed** AND `cargo check --target wasm32-unknown-unknown -p liquers-core` = **green** with tokio `["sync"]` only. Delivered: EvalMode + shared `load_command_versions`; AssetManager trait extended (primitives incl. `remove_expired_from_maps`/B1, `remove_key_asset`/`insert_key_asset`/`get_envref`/`untrack_expiration`/`next_id_for_asset`) + shared default methods (Q1 deps + all 12 store-delegating methods); `Environment::AssetManager` assoc type; `AssetRef::run_inline` (futures::join!/select!); `ImmediateAssetManager` (Mutex maps, lazy start/expiry); wasm cfg-gating of all Queued spawn/timer carriers (incl. B2 persist, MetadataSaver, cancel, new_temporary) + SimpleEnvironment*/sync-Store/Cache cfg-out (Q2); `.boxed()`→`maybe_boxed()`; `PsmJoinError` alias.
- **M-C DONE (green, committed) ✅** — macro emits `maybe_boxed`/`BoxFuture` (register_command! + register_lui_commands!); `DefaultEnvironment` cfg-selects `ImmediateAssetManager` on wasm (+ `init_with_envref` split, `start()`, `Arc<Box>`→`Arc`); liquers-py/axum conformance (deref fix); A2 `Instant::now()`/`sleep` gated in query console. **Native workspace green; 324 core tests pass; `ui_spec_demo` compiles to wasm32-unknown-unknown (0 errors) — the full browser chain builds.**
- **M-D DONE (green, committed) ✅** — `ImmediateEnvironment` (test-support) + `tests/manager_parametric.rs`: same `AssetManager` contract over both managers, immediate concurrency-dedup, and the **no-tokio-runtime proof** (`futures::executor::block_on`, no runtime). Native persist made synchronous in `Inline` mode so immediate eval is fully spawn-free on native too. **All 6 parametric tests pass; full core suite (324 lib + integration) green; wasm compiles. ImmediateAssetManager proven correct at runtime.**
- **M-E DONE (green, committed) ✅ — ACCEPTANCE MET.** `trunk build` produces the wasm bundle (wasm-bindgen-cli 0.2.126); Playwright e2e (`tests/webui.spec.ts` + `playwright.config.ts`) drives `ui_spec_demo` in headless Chromium: renders "Add Dashboard", clicks → `evaluate_immediately` → inline `ImmediateAssetManager` eval → re-render shows "Dashboard", **zero pageerror/console errors — 1 passed.** liquers-core runs in a real browser; the deferred webui M4 goal is met.
- **M-F DONE ✅** — docs updated (this file, `webui/DESIGN.md`, `ISSUES.md`, `PROJECT_OVERVIEW.md`).

## STATUS: IMPLEMENTATION COMPLETE ✅
All milestones M-A…M-F delivered, green, committed, pushed. `liquers-core` + `liquers-lib` + macro + `ui_spec_demo` compile to `wasm32-unknown-unknown`; native workspace green with 324+ core tests + 6 parametric tests (incl. no-tokio-runtime proof) passing; wasm tokio reduced to `["sync"]`; Playwright e2e passes in real Chromium. Out of scope (tracked in ISSUES): full tokio removal / executor-agnostic core; Tier-2 browser-native IndexedDB/fetch store + JS-closure command backend.
- **(superseded) M-B REMAINING notes:**
