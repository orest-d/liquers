...
1. **Scope:** ~~Tier 1 only or Tier 1 + Tier 2?~~ **RESOLVED (user, 2026-07-22): EXPANDED to BOTH blockers together.** Initially "Tier 1 first"; the user then directed a complete solution covering Axis 1 (behavior, via b1 `ImmediateAssetManager`) **and** Axis 2 (the `Send` bound, via target-gated conditional compilation), because the two are co-dependent on wasm: once `Send` is relaxed, `tokio::spawn` in `DefaultAssetManager` fails to *compile*, forcing the threaded manager out of the wasm build (`#[cfg(not(wasm32))]`) with `ImmediateAssetManager` as its replacement. **Success criterion: `ui_spec_demo` runs in a browser and passes a Playwright e2e test.** *Note:* b1 achieves Axis 1 **without any `rt` shim** — the inline path has no spawns/timers at all. Phase 2 covers both axes.
2. **Threading model:** ~~single-thread vs keep-Send?~~ **RESOLVED: single-threaded wasm** — Axis 2 drops `Send` on `wasm32` via `MaybeSend`/`MaybeSync` markers; native keeps full `Send + Sync`.
3. **Shim vs. crate:** ~~cfg shim vs `tokio_with_wasm`?~~ **RESOLVED: neither** — b1's inline path has no spawns/timers, so no `wasm-bindgen-futures`/`gloo-timers`/`tokio_with_wasm` is needed in `liquers-core`.
...
- `specs/design/webui/DESIGN.md`, `specs/archive/2026-08-08-issues.md` — prior session's async-on-wasm follow-up notes.
