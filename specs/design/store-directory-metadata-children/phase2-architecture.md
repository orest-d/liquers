...
Keep children populated, matching the trait default and seven implementations; document the cost and align AsyncOpenDALStore or explicitly scope its exception.

## Integration Boundary

**Files and symbols:** specs/reference/STORE_SEMANTICS.md, liquers-core/src/store.rs, liquers-store/src/opendal_store.rs, liquers-web/src/store/local_storage.rs, liquers-core/tests/store_conformance_CONF.rs. Reuse existing typed Error constructors and existing async traits; avoid new ownership or dispatch abstractions unless the named boundary requires them. Serialized additions are optional and additive; public Rust renames retain explicit compatibility handling where stated.
