...
1. Inspect the current signatures and callers in liquers-core/src/store.rs, liquers-core/tests/store_conformance_CONF.rs; stop if they differ from Phase 2. Proof: the focused Phase 3 test. Containment: revert only this source's files.
2. Implement Make an absent member prefix enumerate as an empty namespace at the AsyncFileStore/listdir boundary, retaining other errors. Preserve existing ownership, async, serialization, and typed-error conventions. Proof: cargo test -p liquers-core --test store_conformance_CONF.
