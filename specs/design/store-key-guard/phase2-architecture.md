...
| `STORE-ABSOLUTE-KEY-NOT-TYPE-ENFORCED` | draft | P3 | Filed **by** this design: the rejected `AbsoluteKey` newtype. Records that this design enforces the precondition by convention, not by signature. | no | no | None — it is the successor, not a prerequisite. | New, P3 |
...
|---|---|
| `AbsoluteKey` newtype; stores take `&AbsoluteKey` | Makes forgetting *impossible* rather than visible — genuinely stronger. Rejected for cost: every method of both traits, both routers, all six stores, `liquers-py` and `liquers-web` signatures — a cross-crate API break inside a P0 security fix. **Filed as `specs/issues/STORE-ABSOLUTE-KEY-NOT-TYPE-ENFORCED.md`** (P3, complexity L, so it needs its own design folder under §4.5) rather than left in this table. Revisit if the evidence above says the manual check is being forgotten. |
