...
> **Revised 2026-09-02, twice.** The first draft addressed three defects. The second reproduction
> recorded in Phase 1 added two — one destructive — and disproved this document's own claim that
> `make_sub_dirs` satisfies the `//TODO: create_dir` markers. The document was then restructured to
> the `liquers-project` template when that workflow was adopted at the gate, and the gate's four
> decisions folded in.
...
|---|---|---|---|
| 1 | `PathMap` / `DecodedPath`, every call site routed through it | 1, 2, 5 | 1 |
| 2 | `key_prefix()` returns the configured prefix | 3 | 2 (own commit, Q2) |
| 3 | Directory keys on backends with no directory objects | 4 | 3 |
| 4 | `removedir` doc comment corrected | 1 | 1 |
| 5 | `make_sub_dirs` deleted; dead synchronous block deleted | 6, Q3 | 4 |
| 6 | Warning hygiene; two folded-in P3 issues | — | 5 |

All of it lands in `liquers-store`: one source file and its colocated tests, plus one `#[cfg]` line
in `store_factory.rs`.
...
| `STORE-ASYNC-STORE-NO-BEHAVIOURAL-CONFORMANCE-SUITE` | P1/L | draft (**filed by this design**, 2026-09-02) | Four of the six defects are divergences from what `AsyncMemoryStore` and `AsyncFileStore` already do; one shared suite would have caught them. Out of scope here (an `L` change to `liquers-core`'s test surface inside a P0 fix), but the *contract* it would encode is written by this design's Documentation Architecture. | No |
...
- `has_children` is `async` because it awaits `Operator::list_with`. It is the only new `async`
  function.
...
`contains` (`:414`) gains the same fallback `AsyncMemoryStore` has — data, else metadata, else
...
`get_metadata("sub")` fails — and `AsyncStore::get_asset_info` (`store.rs:405`) starts with
...
Synthesising from the listing is what both `liquers-core` memory stores do, it is the only
definition that works uniformly, and it makes `listdir` and `is_dir` agree by construction — which
is exactly the inconsistency Phase 1 found. **Cost:** one extra listing call, only on the path that
today returns an error, asking for one page.
...
| `liquers-core/src/store.rs` | no | but `AsyncStoreRouter::is_dir` (`:2053`), `listdir` (`:2080`) and `listdir_keys_deep` change *behaviour* through `key_prefix()` — the impact crosses a crate boundary though the edit does not |
...
| `liquers-web` | no | no longer depends on `liquers-store` at all |
...
**Feature gates:** every changed symbol is already inside `#[cfg(feature = "async_store")]`, so no
new gate is needed; §6 is the one place a gate changes. `scripts/check-build-matrix.sh` must stay
green, including the `opendal`-off configuration and the `opendal`-without-`async_store` row that §6
unblocks.
...
|---|---|---|---|---|
| `specs/reference/STORE_SEMANTICS.md` | reference | **create** | internal — anyone implementing or calling an `AsyncStore` | The store behavioural contract that four of the six defects violate: what `is_dir` and `contains` mean when the backend has no directory objects; that `is_dir` on an absent key is `Ok(false)`, not an error; that `removedir` is recursive and **scoped to the directory**, so no operation on a key may reach a sibling key; that removing an absent directory is a no-op; the prefix convention (a store's `key_prefix` is part of the path under its backend root, and `key_prefix()` must report it). Written at Phase 5 against implemented and tested behaviour. |
...
| `specs/issues/STORE-ASYNC-STORE-NO-BEHAVIOURAL-CONFORMANCE-SUITE.md` | issue | **created** 2026-09-02 | — | The suite that would enforce `STORE_SEMANTICS.md` across all four implementations. `L`, hence filed not folded. |
...
Generated candidates by `area: [store/backends]` also surface `reference/ENVIRONMENT_CONFIG.md` and
`guides/STORE_FACTORY_GUIDE.md`; both describe configuration and factories, neither describes store
semantics, so both are reviewed at Phase 5 and expected to be dropped from the authoritative set.
...
| Build the shared `AsyncStore` conformance suite here | Rejected and filed: an `L` change to `liquers-core`'s test surface would swamp a P0 fix. |
...
- **No public signature changes.** `key_to_path`, `key_to_path_metadata` and `path_to_key` keep
  their types. `key_to_path` gains one refusal (a filename ending in the metadata suffix), which
  `is_supported` already refused. `make_sub_dirs` was private. The deleted synchronous block is
  commented out and therefore not API at all.
...
| Assessment | Record |
|---|---|
| **Files** | 2 source files in one crate: `opendal_store.rs` (implementation and colocated tests) and one `#[cfg]` line in `store_factory.rs`. ~150 lines added, ~60 changed, ~225 deleted (of which 200 are the dead synchronous block). Specs: `specs/README.md`, the issue file, two folded-in issue files, `phase5-documentation.md`, the new `STORE_SEMANTICS.md`, `specs/index.csv`. No generated or configuration files. |
...
| **Recovery** | Five independent commits, each reverting alone: `PathMap` and the trailing slash (§1, §4), `key_prefix()` (§2), the directory fallback (§3), the deletions (§5), hygiene and folded-in issues (§6). `key_prefix()` in particular is a one-line revert if a routing regression appears in the field. The dead-block deletion is recoverable from git history. Nothing is persisted, so revert needs no migration. |
| **Certainty** | High on the mechanism: every claim was executed against both backends, not inferred, and both fixes were verified at the operator level before being proposed. All four gate questions are answered. Unverified: the trailing-slash behaviour of `list_with`/`remove_all` on a *remote* object store (S3, GCS) — probed on `memory` and `fs`, the two shapes available offline; OpenDAL's prefix-versus-directory semantics are backend-independent by design, and the risk of a remote backend differing is low and in the safe direction (a narrower scope than today's). |
...
clearance limit of three on its own, and the change is P0 with two behavioural changes of external
reach. This work does not clear a gate automatically and does not claim to.
