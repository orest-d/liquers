...
**Code changes:** As drafted in `specs/design/expiration-safety/phase3-examples.md` "Test Plan > Unit
...
in `specs/design/expiration-safety/phase3-examples.md`, verified compilable against the real APIs added
in Steps 1-3.
...
- **Knowledge:** All files touched in Steps 1-5; `plan20260707.md`'s "Validation commands"
  convention; `specs/design/async-wasm-refactor/DESIGN.md` for the wasm-check convention
...
- `EXPIRATION-SAFETY.md`: change `Status: Draft` to `Status: Closed`; add a short "Implemented via
  `specs/design/expiration-safety/` (WP-3)" pointer, noting the acceptance criteria this WP adds beyond
...
- `specs/design/expiration-safety/DESIGN.md`: mark all phases complete, add final implementation notes/
  links to the merged PR, and note the `async-wasm-refactor` sync that happened mid-Phase-4.

**Validation:**
```bash
git diff specs/archive/EXPIRATION-SAFETY.md specs/archive/EXPIRATION-SAFETY-IMPLEMENTATION-PLAN.md specs/design/expiration-safety/DESIGN.md
# Expected: status fields updated, no other content regressions
```

**Rollback:**
```bash
git checkout specs/archive/EXPIRATION-SAFETY.md specs/archive/EXPIRATION-SAFETY-IMPLEMENTATION-PLAN.md specs/design/expiration-safety/DESIGN.md
```
...
```
Update `specs/design/expiration-safety/DESIGN.md`'s "Notes" section with which steps are done; resume by
...
4. **Exit** — this document is saved at `specs/design/expiration-safety/phase4-implementation.md` for
   manual execution.
