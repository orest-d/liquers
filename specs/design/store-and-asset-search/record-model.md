...
| **An inline projection** — running a command per candidate during the search | forbidden; see below | — |

### Inline projection is forbidden by the non-evaluation invariant

Running a projection command per candidate is an evaluation, and a search that evaluates can
recompute a corpus from one query. The invariant is not negotiable for a search over a corpus of
unknown size, so:

> **A search reads projections that already exist. It never creates one.**

Materializing a projection is a separate, deliberate act — a recipe, or an explicit command run once
— and the asset layer then keeps it fresh by content hash, re-deriving exactly the documents that
changed. A document whose projection has not been materialized simply has no projected fields; it is
still findable by text and by key.

This is the same shape as the interoperability layer, with the built-in case being the degenerate one
where the sink is Liquers' own store. It is also, encouragingly, what this repository already does by
hand: `specs/index.csv` is a materialized projection of every document's front-matter, regenerated by
a script and committed. The practice preceded the principle.
