...
## An engine may decline part of a schema
...
So the sink contract requires an engine to **report what it did not index**, per field and with a
reason, when a stream is first fed to it. The feed result carries that report; a caller can then pair
