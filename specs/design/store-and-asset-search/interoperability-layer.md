...
## An engine reports what it did with each field: exact, inexact, or not at all

**Added 2026-09-20, generated by `record-streams` Phase 2 revision 6.** Writing the per-engine
mapping for the composable `FieldRole` produced a requirement this layer did not have.

A field's `FieldRole` declares *portable intent* — searchable how, stored, fast. Not every engine can
serve every intent: **Tantivy has no first-class vector index**, so a `Similarity` field handed to a
Tantivy sink cannot be indexed as asked. Silently dropping it is the worst outcome, because a later
similarity query returns nothing with no indication why.

So the sink contract requires an engine to **report what it did with each field** when a stream is
first fed to it. Two outcomes are not enough, and DataFusion shows why: its `TableProvider` reports
per filter one of **`Exact`**, **`Inexact`** or **`Unsupported`**, where `Inexact` means the source
narrowed the candidates but the engine **must re-apply the filter** to be correct.

That third state is exactly what an external search engine needs and what an indexed/declined pair
cannot express — an engine whose tokenization does not match the predicate returns a superset, and
the caller must verify. It is the **filter-then-verify** shape this design already wants. So the
report is three-valued:

| Outcome | Meaning |
|---|---|
| `Exact` | the engine applied the intent fully; results need no re-checking |
| `Inexact` | the engine narrowed the candidates; **the caller must re-apply the predicate** |
| `Unsupported` | the engine did nothing with this field | The feed result carries that report; a caller can then pair
the engine with a second one (a vector store beside a text index, which is the normal architecture
anyway), or accept the gap knowingly.

Two related per-sink obligations, recorded here rather than in the record schema, which must stay
free of engine specifics:

- **Engine-specific options live in the sink's configuration**, keyed by field name, layered over the
  schema's intent — a registered tokenizer name, HNSW parameters, a custom analyzer.
- **Qdrant point ids must be unsigned integers or UUIDs.** An arbitrary string record id does not
  fit, so that sink hashes or maps ids and keeps the original in the payload.
