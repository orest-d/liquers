...
The constraint that decides the shape is the non-evaluation invariant. Running a projection command
per candidate during a search is an evaluation, and a search that evaluates can recompute a corpus
from one query. So:

> **A search reads projections that already exist; it never creates one.**

Which leaves two routes to `status`, both of which put the work *outside* search:

| Route | Mechanism | Cost to search |
|---|---|---|
| **Into metadata** | a recipe runs the extraction command and writes the fields into the asset's metadata; needs `CORE-METADATA-NO-APPLICATION-ATTRIBUTES` | **none** — field predicates already resolve by name against the record |
| **Into a companion asset** | the extraction result is its own derived asset that search consults for each candidate | a lookup convention, a per-candidate read, and sidecar-key hazards |

**Recommendation: the metadata route**, and therefore no change to the search MVP at all. The
extraction command belongs to agent memory, which already plans recipes to populate `title` and
`description`; adding front-matter fields to the same recipe is a small extension of work already
scoped there. This also keeps M1's rule intact — field predicates resolve **by name against the
record**, never against `MetadataRecord`'s struct fields.

Two honest consequences:

1. **A3's best form does depend on `CORE-METADATA-NO-APPLICATION-ATTRIBUTES` (P2) landing.** Without
   it, the MVP filters by key prefix and text — 72 → 42 on the measurement above — and "still open"
   is unavailable. That is a degraded MVP, not a blocked one.
2. **The repository already validates the shape.** `specs/index.csv` is a materialized projection of
   every document's front-matter, regenerated by a script and committed, precisely because computing
   it per query would be wasteful. The practice preceded the principle. Searching *that file* as a
   record source would work, but it is Level 1 — rows from a CSV — which is a good reason not to
   reach for it in the MVP.
