...
Derive enabled service config fields from Default plus Serialize behind per-service cfg gates, then add offline S3 construction tests.

## Integration Boundary

**Files and symbols:** liquers-store/src/store_factory.rs, liquers-store/src/opendal_store.rs, liquers-store/Cargo.toml. Reuse existing typed Error constructors and existing async traits; avoid new ownership or dispatch abstractions unless the named boundary requires them. Serialized additions are optional and additive; public Rust renames retain explicit compatibility handling where stated.
