...
| `save_to_store` might refuse a non-`Ready` status | **Checked — it does not.** `save_to_store` (`assets.rs:2604`) gates only on cancellation and keyedness; it reads bytes through `binary_unchecked` precisely so the read gate does not apply. Writing `Expired` needs no change there |
...
| P4 | **Assuming the store refuses a non-`Ready` status** | Defensive code that special-cases the write, or a "fix" that skips writing entirely | Guesswork instead of reading `save_to_store` | **Verified: there is no status gate.** `save_to_store` (`:2604`) checks cancellation and keyedness only | I2: the entry exists and says `Expired` |
