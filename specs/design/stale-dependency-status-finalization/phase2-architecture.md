...
| `EXPIRY-RECORDS-NO-REASON` | draft | P2 | Every route into `Expired` is silent about why. This design writes its reason *before* persistence, which is the ordering that issue must follow for the other routes | no | Monitor; the precedent is set here |
