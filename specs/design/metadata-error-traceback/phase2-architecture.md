...
field on `ErrorPayload`; HEAD confirms no existing error-level traceback storage.
...
| `CORE-ERROR-PAYLOAD-SIZE` | closed | P2 | `Error` is boxed at HEAD; preserve its existing pointer-size regression while extending the payload. | no |

## Files and Symbols

Source file: `liquers-core/src/error.rs`, extending `ErrorPayload` with
