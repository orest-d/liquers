...
- Relevant open known issues and blockers are resolved or made explicit prerequisites
...
Search before finalizing the solution:
...
- Mark an issue **blocking** when the project cannot be implemented safely or correctly until it is
  resolved, or when it invalidates a necessary architecture assumption, dependency, or interface.
- A blocking issue is prerequisite work. Resolve it first or redesign the solution so the project
  no longer depends on it. Do not request Phase 2 approval with an unresolved blocker.
- Review the priority of every blocker. It must be at least `P1`; recommend or apply an increase
  from `P2`/`P3` after confirming the change with the user or issue owner.
- Use `P0` only when the issue also meets `DOCS_STRUCTURE_GUIDE.md` §4.4: incorrect results, data
  loss, a panic on a supported path, or a documented feature that does not work. Blocking this
  project by itself establishes `P1`, not automatically `P0`.
- For a relevant non-blocker, explain how the architecture tolerates, works around, or remains
  independent of it. Link follow-up work rather than hiding the impact.
...
Fully specify every Phase 1 documentation decision. Do not leave candidate names or broad document
categories unresolved at the Phase 2 approval gate.

### Reference Plan

State whether to create a new reference, extend an existing reference, or create none. Give the
exact path, audience, area, purpose, sections or claims to add/change, and required cross-links.

### Guide Plan

State whether to create a new guide, extend an existing guide, or create none. Give the exact path,
audience, area, task/workflow covered, planned examples or snippets, and required cross-links.

### Other Documents to Create

List every other new document with its exact path, kind, audience, purpose, and link destination; or
write `None` with the Phase 1 rationale.

### New Reference or Guide Documents

List exact paths for every new current-state document approved in Phase 1:

| Path | Kind | Audience | Area | Purpose |
|---|---|---|---|---|
| `specs/reference/<NAME>.md` | reference | internal/user/both | `<area>` | Explain current behavior, meaning, importance, and connections |
| `specs/guides/<NAME>.md` | guide | internal/user/both | `<area>` | Explain how to perform or extend the work |

Use `SCREAMING_SNAKE_CASE.md`. If no new document is needed, write `None` and preserve the Phase 1
rationale. Do not use the design as a substitute for current documentation.

### Existing Documents to Review or Update

List the exact documents selected in Phase 1 plus candidates selected by `area`. Decide which
belong in authoritative `affects_docs`, state the precise claims, instructions, or links to change,
and explicitly record discarded candidates. Include front-matter `reviewed:` and `## History`
updates required by `DOCS_STRUCTURE_GUIDE.md` §9.

### Design and Capability Links

Identify every place where design artifacts must be linked during design, or replaced by a
reference/guide after documentation. Include the capability entry in `specs/README.md` and any
cross-links needed to anchor the new functionality in existing documentation.

### Evidence to Collect During Implementation

List usage guidance, developer workflow, corrections, connections, and unexpected learning to
capture during Phases 3-4 and implementation for Phase 5.
...
Searched the issues linked from the design, `area: commands/polars`, the `liquers-lib` and
`liquers-axum` integration points, and open dependency/serialization issues in `specs/index.csv`.
No relevant open issue was found.

### Blocking and Priority Decision

No prerequisite or blocking issue was identified, so no priority change is required. If the Polars
version or feature-flag concern described under Compilation Validation is filed as an issue later,
reassess it before Phase 2 approval rather than treating this search result as permanent.
...
### Known-Issue Preflight
- [ ] Linked, area-overlapping, integration, dependency, and assumption-related issues were searched
- [ ] Each relevant issue has a documented solution impact and prerequisite/blocking decision
- [ ] `None found` states what was searched
- [ ] No unresolved blocker remains; every blocker is at least P1, with P0 reserved for §4.4 impact
...
### Documentation Architecture
- [ ] Phase 1 reference/guide decision is resolved into exact paths or an explicit `None`
- [ ] New documents have kind, audience, area, purpose, and placement
- [ ] Existing documents to review/update are listed and `affects_docs` candidates identified
- [ ] Capability-map and other design/reference/guide link changes are specified
- [ ] Evidence to collect through implementation is listed
...
- Task: Check Phase 2 against existing code at integration points:
  ...
  - Cross-check the known-issue preflight against affected areas and integration points
  - Flag missing blockers, understated solution impact, or blocker priority below P1
