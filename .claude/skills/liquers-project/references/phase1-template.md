...
**Reference:** <Create `specs/reference/<NAME>.md`, extend an existing reference at an exact path,
or neither — explain why>

**Guide:** <Create `specs/guides/<NAME>.md`, extend an existing guide at an exact path, or neither
— explain why>
...
reading this design. A reference explains current behavior, meaning, and connections; a guide
explains how to perform a repeatable task. Reconsider `neither` later if substantial learning
accumulates. Follow `specs/DOCS_STRUCTURE_GUIDE.md` §2, §8, and §9.>
...
- [ ] **Is the feature appropriately scoped (not too large, not trivial)?**
  ...
  - If too small: This might not need the full 5-phase process

### Documentation Intent
- [ ] **Reference is explicitly new, extended, or not required**
- [ ] **Guide is explicitly new, extended, or not required**
- [ ] **Other documents to create are identified, or `None` is justified**
- [ ] **Specific documents to update are identified, or `None` is justified**
- [ ] **Every decision has a brief rationale tied to current behavior versus repeatable work**
- [ ] **The intended audience and documentation outcome are clear**
- [ ] **A `neither` decision includes the condition that would cause reconsideration**
