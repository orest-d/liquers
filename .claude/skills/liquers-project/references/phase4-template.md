...
Carry the Phase 2 documentation architecture into explicit implementation and Phase 5 steps.

### New Reference and Guide Documents

- Exact `specs/reference/` and `specs/guides/` paths
- Purpose, audience, area, and responsible implementation step
- Information that must be captured while implementing

### Existing Documents and `affects_docs`

- Candidate documents selected by `area`
- Expected authoritative `affects_docs` set
- Claims or instructions to verify against implemented and tested behavior
- Required `reviewed:` and `## History` updates

### Design, Capability, and Cross-Links

- `specs/README.md` capability link during design and after documentation
- Links to add, replace, or remove when a current reference/guide becomes authoritative
- Cross-links that anchor meaning and use in existing documentation

### Phase 5 Evidence Capture

For every implementation and review step, capture requested-versus-implemented scope, omitted or added
work and reasons, filed issues, developer guidance, connections, corrections, and unexpected
learning. Phase 5 synthesizes this evidence; it should not rediscover it.
...
- [ ] Documentation can be verified against the implemented and tested behavior
- [ ] Phase 5 documentation will be included in the implementation PR when practical
- [ ] Documentation and learning evidence has been collected
...
6. Create pull request and resolve all review feedback
7. After implementation is finished, validated, and review feedback is addressed, complete Phase 5
   before merge when practical
8. If rebase or merge conflicts change relevant content, recheck documentation consistency after merge
